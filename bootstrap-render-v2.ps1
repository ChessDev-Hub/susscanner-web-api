...
RUN THIS from the ROOT of the repo that's connected to Render (sus-scanner-api / SusScannerWebApi).
#>
...
#   python -m uvicorn api.main:app --reload --host 127.0.0.1 --port $LocalPort
"@
...
Write-Host "   python -m uvicorn api.main:app --reload --host 127.0.0.1 --port $LocalPort"
